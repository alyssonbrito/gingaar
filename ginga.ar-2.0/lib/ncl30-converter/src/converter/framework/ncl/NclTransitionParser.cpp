...
void* NclTransitionParser::parseTransitionBase( DOMElement* parentElement ) {
	...
	parentObject = createTransitionBase( parentElement );
	...
	for (int i = 0; i < (int) elementNodeList->getLength(); i++) {
		...
		if (node->getNodeType() == DOMNode::ELEMENT_NODE) {
			...
			if (elementTagName.compare( "importBase" ) == 0) {
					...
					addImportBaseToTransitionBase( parentObject, element);
			} else if (elementTagName.compare( "transition" ) == 0) {

				elementObject = parseTransition( element );
...
void *NclTransitionParser::parseTransition( DOMElement *parentElement ) {

	return createTransition( parentElement );
}
...
void NclTransitionParser::setImportParser( NclImportParser *importParser ) {
