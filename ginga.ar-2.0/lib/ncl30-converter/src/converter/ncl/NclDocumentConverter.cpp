...
const std::string &NclDocumentConverter::getAttribute( void* element, const std::string &attribute ) {
	...
	std::string *attr = new std::string(xmlHndl->getStr(((DOMElement*) element)->getAttribute( xmlHndl->getXMLCh( attribute))));
	return *attr;
}
...
void* NclDocumentConverter::parseRootElement( DOMElement* rootElement ) {
	...
	if (parseEntityVar) {
		elementName = getXmlHandler()->getStr( rootElement->getTagName() );
