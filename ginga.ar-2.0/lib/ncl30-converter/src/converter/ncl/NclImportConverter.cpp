...
namespace converter {
namespace ncl {
NclImportConverter::NclImportConverter( bptcf::DocumentParser *documentParser ) :
		NclImportParser( documentParser ) {

}

bool NclImportConverter::addImportNCLToImportedDocumentBase( DOMElement *childObject ) {

	std::string docAlias, docLocation;
	NclDocumentConverter *compiler;
	bptn::NclDocument *thisDocument, *importedDocument;
	bptcf::XMLChHandler *xmlHndl = getXmlHandler();
	bool isValid = false;

	// apanha o alias e a localizacao do documento
	docAlias = xmlHndl->getStr(childObject->getAttribute( xmlHndl->getXMLCh( "alias" )));

	docLocation = xmlHndl->getStr(childObject->getAttribute( xmlHndl->getXMLCh( "documentURI" )));

	compiler = (NclDocumentConverter*) getDocumentParser();
	importedDocument = compiler->importDocument( docLocation );
	isValid = (importedDocument != NULL);
	if (isValid) {
