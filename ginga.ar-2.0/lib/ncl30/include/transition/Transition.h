...
public:
	Transition( const std::string &id, int type );
	const std::string &getBorderColor();
	int getBorderWidth();
	short getDirection();
	double getDur();
	double getEndProgress();
	const std::string &getFadeColor();
	int getHorzRepeat();
	...
	void setFadeColor( const std::string &color );
