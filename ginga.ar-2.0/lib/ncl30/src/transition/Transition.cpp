...
Transition::Transition( const std::string &id, int type ) :
		Entity( id ) {
	setType( type );
	this->dur = 1600; /* 1 second */
...
const std::string &Transition::getBorderColor() {
	return borderColor;
}
...
const std::string &Transition::getFadeColor() {
	return fadeColor;
}
...
void Transition::setBorderColor( const std::string &color ) {
...
void Transition::setDirection( short dir ) {
	if (dir >= Transition::DIRECTION_FORWARD && dir <= Transition::DIRECTION_REVERSE) {
...
void Transition::setFadeColor( const std::string &color ) {
...
void Transition::setSubtype( int subtype ) {
	switch ( type ) {
		case Transition::TYPE_BARWIPE:
			if (subtype >= Transition::SUBTYPE_BARWIPE_LEFTTORIGHT && subtype <= Transition::SUBTYPE_BARWIPE_TOPTOBOTTOM) {
		...
		case Transition::TYPE_IRISWIPE:
			if (subtype >= Transition::SUBTYPE_IRISWIPE_RECTANGLE && subtype <= Transition::SUBTYPE_IRISWIPE_DIAMOND) {
		...
		case Transition::TYPE_CLOCKWIPE:
			if (subtype >= Transition::SUBTYPE_CLOCKWIPE_CLOCKWISETWELVE && subtype <= Transition::SUBTYPE_CLOCKWIPE_CLOCKWISENINE) {
		...
		case Transition::TYPE_SNAKEWIPE:
			if (subtype >= Transition::SUBTYPE_SNAKEWIPE_TOPLEFTHORIZONTAL && subtype <= Transition::SUBTYPE_SNAKEWIPE_BOTTOMLEFTDIAGONAL) {
		...
		default:
			if (subtype >= Transition::SUBTYPE_FADE_CROSSFADE && subtype <= Transition::SUBTYPE_FADE_FADEFROMCOLOR) {
