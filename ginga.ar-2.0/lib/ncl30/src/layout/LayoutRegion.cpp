...
if (ncl_util::isPercentualValue( _var )) { \
	var = boost::math::iround( (_parent->method() * ncl_util::getPercentualValue( _var )) - 0.1 ); \
} else { \
	...
	switch ( property ) {
		...
		case ppt::zIndex: {
			try {
				...
				//	-> Table 20 – Reserved parameter/attribute and possible values -> zIndex
				//	-> zIndex : An integer number in the range [0, 255]
				//	Description: Hack for HTML-InfoTetris, element zIndex is defined as "-1"
				//	-> mainLuaJava.ncl: <region id="rgImageLoader" left="0" top="0" width="1" height="1" zIndex="-1"/>
				result = (zvalue >= -1) && (zvalue <= 255);
				if (result) {
					setZIndex( zvalue );
				}
			} catch (boost::bad_lexical_cast &) {
        LWARN("LayoutRegion", "Conversion error by[%s]",value.c_str());
				result = false;
			}
			break;
		}
