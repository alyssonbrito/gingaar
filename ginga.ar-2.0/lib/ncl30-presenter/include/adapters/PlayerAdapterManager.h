...
{
public:
	PlayerAdapterManager( const std::string &docPath, player::System *system, FormatterMediator *formater);
	...
	void onStopTimer( bptgnmc::ExecutionObject* object, bool ms );
	...
	FormatterMediator* getFormatterMediator();
	void setPlayerKeyHandler( bptgnmc::ExecutionObject *object );
	void setFormatterKeyHandler( bptgnmc::ExecutionObject *object );
	void updateGraphicPresentation();

protected:
	typedef boost::shared_ptr<bptgna::FormatterPlayerAdapter> PlayerPtr;
	bptgna::FormatterPlayerAdapter* getPlayer( bptgnmc::ExecutionObject* execObj, bool init=false );
	PlayerPtr initializePlayer( bptgnmc::ExecutionObject* object );
	bool deletePlayer( const std::string &objectId );

private:
	typedef std::map<std::string, PlayerPtr> Players;
	Players _players;
	std::string _docPath;
	player::System *_sys;
	FormatterMediator *_formater;
};
