...
FormatterCausalLink* FormatterLinkConverter::createCausalLink( CausalLink* ncmLink, CompositeExecutionObject* parentObject, int depthLevel, RuleAdapter* ruleAdapter ) {
...
LinkAssessmentStatement* FormatterLinkConverter::createAssessmentStatement( AssessmentStatement* assessmentStatement, Bind* bind, Link* ncmLink, CompositeExecutionObject* parentObject,
	...
	statement = new LinkAssessmentStatement( assessmentStatement->getComparator(), mainAssessment, otherAssessment, ((AttributeAssessment *) (assessmentStatement->getMainAssessment()))->getOffset() );
			...
			if (paramValue == "") {
				...
				if (param == NULL) {
					...
					try{
						...
						LWARN("FormatterLinkConverter", "Conversion error[%s]",param->getValue().c_str());
					}//catch
				}
				...
				try{
					...
					LWARN("FormatterLinkConverter", "Conversion error[%s]",paramValue.c_str());
				}//catch
			}
			...
			if (animation != NULL) {
				...
				if (paramValue[0] == '$') {
					...
					if (param != NULL) {
						...
						try {
							boost::lexical_cast<double>( durValue );
						} catch (boost::bad_lexical_cast &) {
							LWARN("FormatterLinkConverter", "Invalid parameter (duration) for animation[%s]",durValue.c_str());
							durValue = "0";
						}
					}
					APRINT_NCLP("setting duration");
				...
				if (paramValue[0] == '$') {
					...
					if (param != NULL) {
						try {
							...
							LWARN("FormatterLinkConverter", "Invalid parameter (by) for animation [%s]",param->getValue().c_str());
							animation->setBy( "0" );
						}
					} else {
						LWARN("FormatterLinkConverter", "Invalid parameter (by) for animation [null]");
						animation->setBy( "0" );
					}
				}
...
LinkTriggerCondition* FormatterLinkConverter::createSimpleCondition( SimpleCondition* simpleCondition, Bind* bind, Link* ncmLink, CompositeExecutionObject* parentObject, int depthLevel ) {
	...
	event = createEvent( bind, ncmLink, parentObject, depthLevel );
	condition = new LinkTransitionTriggerCondition( event, simpleCondition->getTransition(), _sys, ruleAdapter );
	...
	if (parameter == NULL) {
		...
		try {
			...
			if (param == "") {
				...
				//	LG Electronics: #4228
				//	Original code "boost::lexical_cast<float>( param )" causes the bad lexical cast for delay "5s"
				//	First we need to remove all alphabetical and whitespace characters, if any.
				param.erase(std::remove_if(param.begin(), param.end(), ncl_util::is_alpha_or_space()), param.end());

				return boost::lexical_cast<float>( param ) * 1000;
			}

		} catch (const std::exception &/*exc*/ ) {
      LWARN("FormatterLinkConverter", "Conversion error by [%s]",param.c_str());
			return 0.0;
		}
	}
}
		...
		if (pos != std::string::npos && pos == 0) { // instanceof Parameter
			...
			delay = getDelayParameter( ncmLink, param, bind );
			  ...
			  delay = (double) (boost::lexical_cast<float>( delayObject ));
 	      		...
 	      		LWARN("FormatterLinkConverter", "Conversion error[%s]",delayObject.c_str());
   			}
