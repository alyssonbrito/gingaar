...
void FormatterConverter::addExecutionObject( ExecutionObject* executionObject, CompositeExecutionObject* parentObject, int depthLevel ) {
	...
	if (dataObject->instanceOf( "NodeEntity" )) {
		...
		APRINT_NCLP("GRADSAME ################");
		sameInstances = dataObject->getGradSameInstances();
		if (sameInstances != NULL) {
			APRINT_NCLP("GRADSAME execId[%s]",executionObject->getId().c_str());//instsame apenas
			i = sameInstances->begin();
			while (i != sameInstances->end()) {
				referNode = (ReferNode*) (*i);

				headNode = executionObject->getNodePerspective()->getHeadNode();
				APRINT_NCLP("exec[%s] refer[%s] headNode[%s]",GET_ID(executionObject),GET_ID(referNode),
				headNode->getId().c_str());
				if (headNode->instanceOf( "CompositeNode" )) {
					APRINT_NCLP("execId[%s]",executionObject->getId().c_str());
					if (((CompositeNode*) headNode)->recursivelyContainsNode( referNode )) {
						APRINT_NCLP("execId[%s]",executionObject->getId().c_str());
						addSameInstance( executionObject, referNode );
					}
			...
			if (executionObject->instanceOf( "CompositeExecutionObject" )) {
				...
				if (ncmEventType == EventUtil::EVT_ATTRIBUTION) {
					if (interfacePoint->instanceOf( "PropertyAnchor" )) {
						event = new AttributionEvent( id, executionObject, (PropertyAnchor*) interfacePoint, _eventManager, _sys );

					} else {
						LWARN("FormatterConverter","getEvent, NCM event type is attribution, but interface point isn't");
						event = new AttributionEvent( id, executionObject, NULL, _eventManager, _sys );
					}
				}
				...
				switch ( ncmEventType ) {
					...
					case EventUtil::EVT_SELECTION:
						...
						// Workaround for LGESP-903: App sends only OK but engine does not handle. Change key value.
						if (key == "OK") {
							key = "ENTER";
						}
...
ExecutionObject* FormatterConverter::createExecutionObject( string id, NodeNesting* perspective, CascadingDescriptor* descriptor, int depthLevel ) {
	...
	if (nodeEntity->instanceOf( "ReferNode" ))
	{
		bptn::Entity *referredNode;
		referredNode = ((bptnr::ReferNode*) nodeEntity)->getReferredEntity();

		nodeEntity = (bptnc::NodeEntity*) (referredNode->getDataEntity());
	}	
...
CascadingDescriptor* FormatterConverter::getCascadingDescriptor( NodeNesting* nodePerspective, GenericDescriptor* descriptor ) {
	...
	if (node->instanceOf( "ReferNode" ))
	{
		...
		node = (bptnc::NodeEntity*) (referredNode->getDataEntity());
	}
			...
			if (parentObject->containsUncompiledLink( ncmLink )) {
				...
				if (ncmLink->instanceOf( "CausalLink" )) {
					...
					if (((CausalLink*) ncmLink)->containsSourceNode( dataObject, descriptor ) || contains) {
						...
						formatterLink = ((FormatterLinkConverter*) linkCompiler)->createCausalLink( (CausalLink*) ncmLink, parentObject, depthLevel, ruleAdapter );
...
ExecutionObject* FormatterConverter::processExecutionObjectSwitch( ExecutionObjectSwitch* switchObject ) {
	...
	if (i != executionObjects->end()) {
		...
		switchObject->select( selectedObject, selectedNode );
...
void FormatterConverter::resolveSwitchEvents( ExecutionObjectSwitch* switchObject, int depthLevel ) {
	...
	selectedNode = switchObject->getSelectedNode();
	...
	if (binds != NULL) {
		...
		while (i != binds->end()) {
			...
			if (childObject != NULL) {
				...
				formatterLink = ((FormatterLinkConverter*) linkCompiler)->createCausalLink( link, contextObject, depthLevel, ruleAdapter );
...
void FormatterConverter::eventStateChanged( void* someEvent, short transition, short /*previousState*/ ) {
	...
	if (executionObject->instanceOf( "ExecutionObjectSwitch" )) {
		...
		if (transition == EventUtil::TR_STARTS){
			ExecutionObjectSwitch *switchObject = (ExecutionObjectSwitch*) executionObject;
			vector<FormatterEvent*>* events = switchObject->getEvents();
			vector<FormatterEvent*>::iterator i;
			SwitchEvent* sw=NULL;
			if (switchObject->getSelectedObject()==NULL &&  events) {
				for (i = events->begin();i!= events->end();i++){
					FormatterEvent *e =(FormatterEvent*) (*i);
					if (e->instanceOf("SwitchEvent")){
						sw = (SwitchEvent*)e;
						APRINT_NCLP("SwitchEvent");
					}//if
				}//for
			}//if getSelected
			delete events;
