...
FormatterMediator::FormatterMediator( const std::string &baseId, const std::string &docPath, player::System *system ) {
	...
	playerManager = new PlayerAdapterManager( _docData.docPath, system, this );

	player::settings::Settings::UpdatePlayersCallback fnc = boost::bind( &FormatterMediator::onUpdatePlayersImage, this );
	_sys->settings()->setPlayersImageUpdateCallback( fnc );

	isEmbedded = _docData.parentDocId != "";

	ruleAdapter = new RuleAdapter( system );
...
FormatterMediator::~FormatterMediator() {
	...
	player::settings::Settings::UpdatePlayersCallback fnc;
	_sys->settings()->setPlayersImageUpdateCallback( fnc );
...
bool FormatterMediator::setPropertyValue( string documentId, string nodeId, string propertyId, string value ) {
  APRINT_NCLP("<==> docId[%s] nodeId[%s] propertyId[%s] value[%s]",documentId.c_str(),nodeId.c_str(),propertyId.c_str(),value.c_str());
...
void FormatterMediator::forward() {
}

void FormatterMediator::backward() {
}

TransitionBase* FormatterMediator::getTransitionBase() {
	if (currentDocument != NULL) {
		return currentDocument->getTransitionBase();
	}
	return NULL;
}

void FormatterMediator::onUpdatePlayersImage() {
	if (playerManager) {
		playerManager->updateGraphicPresentation();
	}
}

std::string FormatterMediator::getdocId() {
	return currentDocument->getId();
}

void FormatterMediator::setPEM(PresentationEngineManager *pem) {
	_pem = pem;
}

void FormatterMediator::NCLPlayerNotification(short actionType, std::string propValue) {
	_pem->notifyNCLPlayerEvent(actionType, propValue);
}
