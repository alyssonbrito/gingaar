...
void CascadingDescriptor::updateData( GenericDescriptor* descriptor ) {
	if (descriptor != NULL && descriptor->instanceOf("Descriptor")) {
		cascadeDescriptor( static_cast<Descriptor*>(descriptor) );
	}
}
...
void CascadingDescriptor::cascadeDescriptor( Descriptor* descriptor ) {
	...
	if (descriptor->getExplicitDuration() != "nill") {
	...
	if (descriptor->getKeyNavigation() != NULL) {
		...
		if (keyNavigation->getFocusIndex() != "nill") {
			focusIndex = keyNavigation->getFocusIndex();
		}

		if (keyNavigation->getMoveUp() != "nill") {
			moveUp = keyNavigation->getMoveUp();
		}

		if (keyNavigation->getMoveDown() != "nill") {
			moveDown = keyNavigation->getMoveDown();
		}

		if (keyNavigation->getMoveLeft() != "nill") {
			moveLeft = keyNavigation->getMoveLeft();
		}

		if (keyNavigation->getMoveRight() != "nill") {
			moveRight = keyNavigation->getMoveRight();
		}
	}

	if (descriptor->getFocusDecoration()) {
		if (!_focusDecoration) {
			_focusDecoration = new bptnn::FocusDecoration();
		}
		std::string tmp = descriptor->getFocusDecoration()->getFocusBorderColor();
		if (!tmp.empty()) {
			_focusDecoration->setFocusBorderColor( tmp );
		}
		tmp = descriptor->getFocusDecoration()->getFocusBorderTransparency();
		if (!tmp.empty()) {
			_focusDecoration->setFocusBorderTransparency( tmp );
		}
		tmp = descriptor->getFocusDecoration()->getFocusSelSrc();
		if (!tmp.empty()) {
			_focusDecoration->setFocusSelSrc( tmp );
		}
		tmp = descriptor->getFocusDecoration()->getFocusSrc();
		if (!tmp.empty()) {
			_focusDecoration->setFocusSrc( tmp );
		}
		tmp = descriptor->getFocusDecoration()->getFocusBorderWidth();
		if (!tmp.empty()) {
			_focusDecoration->setFocusBorderWidth( tmp );
		}
		tmp = descriptor->getFocusDecoration()->getSelBorderColor();
		if (!tmp.empty()) {
			_focusDecoration->setSelBorderColor( tmp );
		}
	}
...
const std::string &CascadingDescriptor::getExplicitDuration() {
	return (this->explicitDuration);
}

bool CascadingDescriptor::setExplicitDuration( std::string &dur ) {
	this->explicitDuration = dur;
	return true;
}
...
string CascadingDescriptor::getParameterValue( string paramName ) {
	...
	if (pos != std::string::npos && pos == paramValue.length() - 1) {
      ...
      LWARN("CascadingDescriptor", "Conversion error by[%s]",paramValue.c_str());
		}
...
string CascadingDescriptor::getFocusIndex() {
	return focusIndex == "nill" ? std::string("") : focusIndex;
}

string CascadingDescriptor::getMoveDown() {
	return moveDown == "nill" ? std::string("") : moveDown;
}

void CascadingDescriptor::setMoveDown( const std::string &index ) {
	moveDown = index;
}

string CascadingDescriptor::getMoveLeft() {
	return moveLeft == "nill" ? std::string("") : moveLeft;
}

void CascadingDescriptor::setMoveLeft( const std::string &index ) {
	moveLeft = index;
}

string CascadingDescriptor::getMoveRight() {
	return moveRight == "nill" ? std::string("") : moveRight;
}

void CascadingDescriptor::setMoveRight( const std::string &index ) {
	moveRight = index;
}

string CascadingDescriptor::getMoveUp() {
	return moveUp == "nill" ? std::string("") : moveUp;
}

void CascadingDescriptor::setMoveUp( const std::string &index ) {
	moveUp = index;
}
