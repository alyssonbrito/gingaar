...
void EventTransitionManager::start( double offsetTime ) {
	...
	while (transIx < size) {
		transition = (*transitionEvents)[transIx];
		if (transition->getTime() <= offsetTime && transition->instanceOf( "BeginEventTransition" )) {
	...
	transitionEvents = getTransitionEvents( ContentAnchor::CAT_FRAMES);
	BOOST_FOREACH(transition, *transitionEvents) {
		transition->getEvent()->start();
	}
}
