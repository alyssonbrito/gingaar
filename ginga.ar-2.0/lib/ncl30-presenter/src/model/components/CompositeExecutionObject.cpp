...
ExecutionObject* CompositeExecutionObject::getExecutionObjectRecursivelly( string id ) {
	if (execObjList->empty()) {
		return NULL;
	}
	ExecutionObject* obj;
	map<string, ExecutionObject*>::iterator i = execObjList->begin();
	while (i != execObjList->end()) {
		obj = i->second;
		if (id.compare(obj->getDataObject()->getId()) == 0) {
			return obj;
		}
		if (obj->instanceOf( "CompositeExecutionObject" )) {
			obj = ((CompositeExecutionObject*) obj)->getExecutionObjectRecursivelly(id);
			if (obj) {
				return obj;
			}
		}
