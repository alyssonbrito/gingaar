	...
	switch ( attributeType ) {
		case EventUtil::ATT_NODE_PROPERTY:
			if (event->instanceOf( "AttributionEvent" )) {
				value = getAssessmentWithOffset( ((AttributionEvent*) event)->getCurrentValue() );
			}
			break;
		...
		case EventUtil::ATT_OCCURRENCES:
			value = getAssessmentWithOffset( boost::lexical_cast<string>( event->getOccurrences() ) );
			break;

		case EventUtil::ATT_REPETITIONS:
			if (event->instanceOf( "PresentationEvent" )) {
				value = getAssessmentWithOffset( boost::lexical_cast<string>( ((PresentationEvent*) event)->getRepetitions() ) );
			}
			break;
	}
...
string LinkAttributeAssessment::getAssessmentWithOffset( string assessmentValue ) {

	if (offset != "") {
		try {
			float v1 = boost::lexical_cast<float>( assessmentValue );
			float v2 = boost::lexical_cast<float>( offset );
			return boost::lexical_cast<string>( (int) v1 + v2 );
		} catch (boost::bad_lexical_cast &) {
      LWARN("LinkAttributeAssessment", "Conversion error assessment[%s] offset[%s]",assessmentValue.c_str(),offset.c_str());
		}
	}
