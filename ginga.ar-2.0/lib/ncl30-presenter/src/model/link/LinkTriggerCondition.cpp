...
LinkTriggerCondition::~LinkTriggerCondition() {
	if (_notifyTimerID) {
		_sys->unregisterTimer( _notifyTimerID );
	}
...
void LinkTriggerCondition::conditionSatisfied( void * /*condition*/) {
	...
	_notifyTimerID = _sys->registerTimer( (util::DWORD) delay, boost::bind( &LinkTriggerCondition::notifyConditionSatisfied, this ) );
}

void LinkTriggerCondition::notifyConditionSatisfied( void ) {
	_notifyTimerID.reset();
