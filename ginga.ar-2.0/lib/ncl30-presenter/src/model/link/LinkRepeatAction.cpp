...
namespace model {
namespace link {
LinkRepeatAction::LinkRepeatAction( FormatterEvent* event, short actionType, player::System *system ) :
		LinkSimpleAction( event, actionType, system ) {
...
void LinkRepeatAction::run() {
	if (LinkSimpleAction::event != NULL) {
		if (LinkSimpleAction::event->instanceOf( "PresentationEvent" )) {
			((PresentationEvent*) event)->setRepetitionSettings( repetitions, repetitionInterval );
		}

	} else {
		LWARN("LinkRepeatAction", "run: event == NULL");
