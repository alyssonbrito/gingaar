...
namespace context {
RuleAdapter::RuleAdapter( player::System *system ) {
...
bool RuleAdapter::adaptDescriptor( ExecutionObject* executionObject ) {
	...
	GenericDescriptor* updatedDescriptor;
	...
	if(unsolvedDescriptor != NULL) {
		...
		vector<GenericDescriptor*>* vd = cascadingDescriptor->getNcmDescriptors();
		vector<GenericDescriptor*>::iterator it;
		for(it = vd->begin(); it != vd->end(); ++it) {
			updatedDescriptor = *it;
			if (updatedDescriptor->instanceOf( "DescriptorSwitch" )) {
				descAlternatives = static_cast<DescriptorSwitch*>(updatedDescriptor);
				selectedDescriptor = descAlternatives->getSelectedDescriptor();
				selected = false;
				size = descAlternatives->getNumRules();
				for (i = 0; i < size; i++) {
				...
				if (selectedDescriptor != descAlternatives->getSelectedDescriptor()) {
					cascadingDescriptor->updateData( descAlternatives->getSelectedDescriptor() );
					adapted = true;
				}
			}
		}
...
Node* RuleAdapter::adaptSwitch( SwitchNode* switchNode ) {
	...
	for (i = 0; i < size; i++) {
		...
		if (evaluateRule( rule )) {
			...
			Node *node = switchNode->getNode( i );

			if (node->instanceOf( "ContentNode" )) {
				player::schema::type schema;
				std::string body;

				std::string url = static_cast<ContentNode*>(node)->getUrl();
				player::url::parse( url, schema, body );

				onFetchFilesFromCarousel(url);

				if (schema == player::schema::file && !boost::filesystem::exists( body )) {
					LWARN("RuleAdapter", "Switch resource not found %s", body.c_str());
					continue;
				}
			}
			selectedNode = node;
	...
	if (selectedNode == NULL) {
		...
		if (selectedNode != NULL) {
			//std::string url = static_cast<ContentNode*>(selectedNode)->getUrl();
			std::string url = "";
			if ( selectedNode->instanceOf("ContentNode") ){
				url = ((ContentNode*)selectedNode)->getUrl();
			}
...
bool RuleAdapter::evaluateSimpleRule( SimpleRule* rule ) {
	...
	try {
		attributeValue = _sys->settings()->getProperty( attribute );
	} catch (const std::runtime_error &e) {
		LWARN("RuleAdapter", "Catch exception: %s", e.what());
		return false;
	}
